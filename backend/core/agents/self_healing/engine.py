...
class ErrorClassifier:
    ...
    def classify(cls, error: str, context: dict = None) -> ErrorAnalysis:
        ...
        severity = cls._judge_severity(category, error)
    ...
    @classmethod
    async def classify_with_ai(cls, error: str, context: dict = None) -> ErrorAnalysis:
        """使用 LLM 做深度分类"""
...
{{"category": "network|timeout|assertion|syntax|data|auth|environment|llm|unknown", "severity": "low|medium|high|critical", "summary": "...", "root_cause": "...", "fixable": true|false, "suggested_strategies": ["策略"]}}"""
    ...
    @classmethod
    def _judge_severity(cls, category, _error): 
...
class RetryWithBackoff(FixStrategy):
    ...
    def can_handle(self, a): return a.category in (ErrorCategory.NETWORK, ErrorCategory.TIMEOUT, ErrorCategory.LLM)
    ...
    def execute(self, step_func, params, analysis, max_attempts=3):
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                if not self._is_error(result):
                    return HealingRecord(id=f"h-{int(start*1000)}", step_name=params.get("step_name",""),
        ...
        return HealingRecord(id=f"h-{int(start*1000)}", step_name=params.get("step_name",""),
            ...
            attempts=max_attempts, duration_ms=int((time.time()-start)*1000), notes="全部重试失败")

    def _is_error(self, r): return isinstance(r, dict) and (r.get("status") in ("error","failed") or r.get("error"))
...
class AIFixSyntax(FixStrategy):
    ...
    def can_handle(self, a): return a.category == ErrorCategory.SYNTAX
    ...
    def execute(self, step_func, params, analysis, max_attempts=2):
        ...
        try:
            ...
            if "prompt" in params: params["prompt"] = fixed
            result = step_func(**params)
            success = not (isinstance(result, dict) and result.get("error"))
            return HealingRecord(id=f"h-{int(start*1000)}", step_name=params.get("step_name",""),
                error_analysis=analysis, strategy_used=self.name, success=success, attempts=1,
                duration_ms=int((time.time()-start)*1000), notes="AI修复" if success else "AI修复后仍失败")
        except Exception as e:
            logger.exception(f"[SelfHealing] AI修复异常: {e}")
        return HealingRecord(id=f"h-{int(start*1000)}", step_name=params.get("step_name",""),
            error_analysis=analysis, strategy_used=self.name, success=False,
            attempts=max_attempts, duration_ms=int((time.time()-start)*1000), notes="无法修复")
...
{{"diagnosis":"...","fix_suggestion":"...","modified_params":{{}}}}"""
            ...
            if j >= 0:
                ...
                if diag.get("modified_params"): params.update(diag["modified_params"])
                result = step_func(**params)
                success = not (isinstance(result, dict) and result.get("error"))
                return HealingRecord(id=f"h-{int(start*1000)}", step_name=params.get("step_name",""),
                    error_analysis=analysis, strategy_used=self.name, success=success, attempts=1,
                    duration_ms=int((time.time()-start)*1000), notes=diag.get("fix_suggestion",""))
        except Exception as e:
            logger.exception(f"[SelfHealing] AI诊断异常: {e}")
        return HealingRecord(id=f"h-{int(start*1000)}", step_name=params.get("step_name",""),
            error_analysis=analysis, strategy_used=self.name, success=False,
            attempts=1, duration_ms=int((time.time()-start)*1000), notes="AI诊断后仍无法修复")
...
class SelfHealingEngine:
    ...
    def __init__(self, max_attempts: int = 3, use_ai: bool = True):
        ...
        self._stats = {"total_heals": 0, "successful_heals": 0, "failed_heals": 0, "by_category": {}}
    ...
    def heal(self, step_func: Callable, step_params: dict,
        ...
        if not self._is_error(result):
            return {"status": "success", "result": result, "healing_needed": False, "attempts": 1}
        ...
        self._stats["by_category"][analysis.category.value] = self._stats["by_category"].get(analysis.category.value, 0) + 1
        ...
        for strategy in self._strategies:
            ...
            if record.success:
                ...
                return {"status": "healed", "result": result, "healing_needed": True,
                        "healing_record": record.__dict__, "attempts": record.attempts + 1}
    ...
    def get_history(self, limit=20): return [h.__dict__ for h in self._history[-limit:]]

    def register_strategy(self, s: FixStrategy): self._strategies.append(s)
