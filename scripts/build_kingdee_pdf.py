# -*- coding: utf-8 -*-
"""金蝶 · AI评测工程师 · 定制题库 PDF生成器"""
import os, sys, datetime

DATA_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'gen_kingdee_custom.py')
...
# ---- 封面 ----
pdf = PDF('金蝶 · AI评测工程师 · 定制面试题库')
pdf.set_title('金蝶-AI评测工程师-定制面试题库')
...
pdf.cell(0, 14, safe_text('金蝶 · AI评测工程师'), align='C', new_x="LMARGIN", new_y="NEXT")
pdf.set_font('MSYH', 'B', 20)
pdf.set_text_color(100, 180, 255)
pdf.cell(0, 12, safe_text('定制面试题库'), align='C', new_x="LMARGIN", new_y="NEXT")

# 蓝色标识条
pdf.set_y(62)
pdf.set_fill_color(0, 90, 160)
pdf.rect(15, pdf.get_y(), 180, 28, 'F')
pdf.set_font('MSYH', 'B', 16)
pdf.set_text_color(255, 255, 255)
pdf.cell(0, 8, safe_text(f"L1-L2 深度递进 · {total_qa}道定制题 · 10大模块"), align='C', new_x="LMARGIN", new_y="NEXT")
pdf.set_font('MSYH', '', 12)
pdf.cell(0, 8, safe_text('纯软件赛道 · SaaS+AI · 财务智能化评测方向'), align='C', new_x="LMARGIN", new_y="NEXT")

# 核心覆盖区
pdf.set_y(105)
pdf.set_font('MSYH', 'B', 15)
pdf.set_text_color(25, 50, 90)
pdf.cell(0, 10, safe_text('定制模块覆盖'), align='C', new_x="LMARGIN", new_y="NEXT")
pdf.ln(4)

keywords = [
    ('AutoTestHub深挖', '软件+SaaS测试', 'LLM评测方法论'),
    ('财务AI场景评测', 'RAG系统评测', 'API自动化测试'),
    ('评测指标设计', 'Prompt安全评测', 'CI/CD集成'),
    ('场景题', '综合面试', '财务业务理解'),
]
...
pdf.cell(0, 8, safe_text('专门针对金蝶SaaS+AI方向定制 —— 智能记账/AI审批/财务助手'), align='C', new_x="LMARGIN", new_y="NEXT")
pdf.cell(0, 8, safe_text('无需硬件背景，纯软件赛道，聚焦AI评测核心能力'), align='C', new_x="LMARGIN", new_y="NEXT")
...
pdf.cell(0, 7, safe_text('目标岗位：AI评测工程师 / AI质量工程师 / 软件测试工程师（AI方向）'), align='C', new_x="LMARGIN", new_y="NEXT")
pdf.cell(0, 7, safe_text(f'生成日期：{datetime.date.today().strftime("%Y年%m月%d日")}  |  {total_qa}道深度递进题  |  10大模块'), align='C', new_x="LMARGIN", new_y="NEXT")
...
# ---- 输出 ----
PDF_FILE = os.path.join(OUTDIR, '金蝶-AI评测工程师-定制面试题库.pdf')
pdf.output(PDF_FILE)
file_size = os.path.getsize(PDF_FILE) / 1024 / 1024
print(f"\n{'='*60}")
print(f"[OK] 金蝶AI评测工程师定制题库 PDF生成成功！")
print(f"  文件: {PDF_FILE}")
