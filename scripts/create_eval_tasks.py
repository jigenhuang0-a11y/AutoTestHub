...
BASE = "http://43.163.95.80:8000"
LOGIN = f"{BASE}/api/accounts/login/"
CREATE_TASK = f"{BASE}/api/evaluator/tasks/create_task/"
RUN_EVAL = f"{BASE}/api/evaluator/tasks/run_eval/"

# ========== 登录 ==========
resp = requests.post(LOGIN, json={"username": "admin", "password": "admin123"}, timeout=15)
...
for task_name, model_id, qbank_name in MODEL_PAIRS:
    ...
    payload = {
        "task_name": task_name,
        "task_description": f"对 {model_id} 进行 {qbank_name} 评测，共 {len(questions)} 题",
        "target_type": "custom_api",
        "target_config": {"model_id": model_id, "mode": "chat"},
        "questions": questions,
    }
    resp = requests.post(CREATE_TASK, headers=headers, json=payload, timeout=30)
    if resp.status_code in (200, 201):
        data = resp.json()
        tid = data.get("id")
        created_tasks.append(tid)
        print(f"✅ 创建任务 #{tid} - {task_name}（{len(questions)}题）")
    else:
        print(f"❌ 创建失败 {task_name}: {resp.status_code} {resp.text[:200]}")

print(f"\n📋 共创建 {len(created_tasks)} 个测评任务")

# ========== 执行测评（每5秒一个，避免并发） ==========
print("\n🚀 开始执行测评...")
for tid in created_tasks:
    resp = requests.post(RUN_EVAL, headers=headers, json={"task_id": tid}, timeout=15)
    if resp.status_code in (200, 201):
        print(f"  ✅ 任务 #{tid} 测评已启动")
    else:
        print(f"  ❌ 任务 #{tid} 启动失败: {resp.status_code} {resp.text[:200]}")
    time.sleep(5)

print("\n🎉 全部完成！刷新 AI 测评师页面查看题目和执行结果")
